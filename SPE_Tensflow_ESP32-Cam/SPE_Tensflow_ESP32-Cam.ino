...
//輸入WIFI連線帳號密碼
const char* ssid     = "WLAN_NAME_HIER_EINGEBEN";          //your network SSID
const char* password = "WLAN_PASSWORT_HIER_EINGEBEN";   //your network password

//輸入AP端連線帳號密碼
const char* apssid = "V2-ESP";
const char* appassword = "SICHERES_PASSWORT_EINGEBEN";         //AP密碼至少要8個字元以上
...
void setup() {
  ...
  // Anpassbare Videoframe-Standardgröße (Auflösungsgröße)
  ...
  s->set_framesize(s, FRAMESIZE_QVGA);    //Auflösung UXGA(1600x1200), SXGA(1280x1024), XGA(1024x768), SVGA(800x600), VGA(640x480), CIF(400x296), QVGA(320x240), HQVGA(240x176), QQVGA(160x120), QXGA(2048x1564 for OV3660)

  //s->set_vflip(s, 1);    // Vertikal kippen
  ...
  WiFi.mode(WIFI_AP_STA);  //Alternaitve Modis WiFi.mode(WIFI_AP); WiFi.mode(WIFI_STA);

  //Gibt die statische IP Adresse des Clients an
  //WiFi.config(IPAddress(192, 168, 0, 100), IPAddress(192, 168, 0, 100), IPAddress(255, 255, 255, 0));

  for (int i=0;i<2;i++) {
    WiFi.begin(ssid, password);    //Stellt eine Netzwerkverbindung her
    ...
    while (WiFi.status() != WL_CONNECTED) {
        delay(500);
        if ((StartTime+5000) < millis()) break;    //Wartet 10 Sek. für Verbindungsaufbau
    } 
  
    if (WiFi.status() == WL_CONNECTED) {    //若連線成功
      WiFi.softAP((WiFi.localIP().toString()+"_"+(String)apssid).c_str(), appassword);   // Stellt die SSID so ein, dass die Client-IP angezeigt wird         
      Serial.println("");
      Serial.println("STAIP address: ");
      Serial.println(WiFi.localIP());
      Serial.println("");
  
      for (int i=0;i<5;i++) {   // Wenn mit WIFI verbunden sind, wird der Blitz so eingestellt, dass er schnell blinkt
        ledcWrite(4,10);
        delay(200);
        ledcWrite(4,0);
        delay(200);    
  ...
  if (WiFi.status() != WL_CONNECTED) {    //Wenn die Verbindung fehlschlägt
    WiFi.softAP((WiFi.softAPIP().toString()+"_"+(String)apssid).c_str(), appassword);         

    for (int i=0;i<2;i++) {    //Wenn keine WLAN-Verbindung herstellt werden konnte, wird der Blitz so eingestellt, dass er langsam blinkt
      ledcWrite(4,10);
      delay(1000);
      ledcWrite(4,0);
      delay(1000);    
  ...
  //Gibt die Access-Point IP Adresse an
  ...
  Serial.println("Local Adress:");
  Serial.println(WiFi.localIP());
  Serial.println("");
  
  startCameraServer(); 

  //Stellt den Blitz auf niedrige Stufe
  pinMode(4, OUTPUT);
  digitalWrite(4, LOW); 
}
...
// Steuerung der Befehlsparameter
static esp_err_t cmd_handler(httpd_req_t *req){
    char*  buf;    // Zugriff auf den Parameterstring nach der URL
    size_t buf_len;
    char variable[128] = {0,};  // Zugriff auf die Werte der Parameter var
    char value[128] = {0,};     // Zugriffsparameter val
    String myCmd = "";
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
          if (httpd_query_key_value(buf, "var", variable, sizeof(variable)) == ESP_OK &&
            ...
            myCmd = String(buf);   //Wenn das inoffizielle Format nicht var, val enthält, handelt es sich um ein benutzerdefiniertes Befehlsformat
          }
        }
        free(buf);
    } else {
    ...
    if (myCmd.length()>0) {
      myCmd = "?"+myCmd;  // Konvertieren Sie die Parameterzeichenfolge nach der URL in ein benutzerdefiniertes Befehlsformat
      for (int i=0;i<myCmd.length();i++) {
        getCommand(char(myCmd.charAt(i)));  // Disassemblierung der benutzerdefinierten Befehlsparameterzeichenfolge
      }
    }
    ...
    if (cmd.length()>0) {
      ...
      if (cmd=="ruediger") {
        // You can do anything
        Feedback="<font color=\"red\">Vsauce Micheal here</font>";
        Feedback += "<br>";
        Feedback += "<h2>SPE Bilderkennungs IDE - with Teachable Maschine</h2>";
      }
      else if (cmd=="ip") {  //Erhalte die AP-IP & die STA-IP
        Feedback="AP IP: "+WiFi.softAPIP().toString();    
        Feedback+="<br>";
        Feedback+="STA IP: "+WiFi.localIP().toString();
      }  
      else if (cmd=="mac") {  //Abfrage der MAC-Adresse
      ...
      else if (cmd=="resetwifi") {  // Netzwerkverbindung zurücksetzen   
        for (int i=0;i<2;i++) {
          ...
          if (WiFi.status() == WL_CONNECTED) {
            ...
            for (int i=0;i<2;i++) {    // Wenn keine Verbindung zum WIFI herstellt werden konnte, stellen Sie das Blinklicht so ein, dass es langsam blinkt.
              ledcWrite(4,10);
              delay(300);
              ledcWrite(4,0);
              delay(300);    
      ...
      else if (cmd=="flash") {  //Steuerung des eingebauten Blinklichts
        ledcAttachPin(4, 4);  
        ledcSetup(4, 5000, 8);   
        int val = P1.toInt();
        ledcWrite(4,val);  
      ...
      if (Feedback=="") Feedback=Command;  // Wenn keine Datenrückgabe eingestellt ist, wird der Befehlswert zurückgegeben.
    
      const char *resp = Feedback.c_str();
      httpd_resp_set_type(req, "text/html");  // Einstellung des Formats der Rückgabedaten
      httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");  // Domänenübergreifende Lesevorgänge zulassen
      return httpd_resp_send(req, resp, strlen(resp));
    } 
    else {
      //Offizieller Befehlsblock, Sie können Ihre eigenen Befehle hier anpassen http://192.168.xxx.xxx/control?var=xxx&val=xxx
      int val = atoi(value);
      sensor_t * s = esp_camera_sensor_get();
      int res = 0;
      ...
      if (buf) {
        ...
        httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
        return httpd_resp_send(req, resp, strlen(resp));  //Rückgabeparameter String
      }
      else {
        httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
//Anzeige des Status der Videoparameter (json-Format muss zurückgegeben werden, um die Anfangseinstellungen zu laden)
